...
module.exports = {
    ...
    getExpiresIn() {
        return JSON.parse( fs.readFileSync(data_path).toString() ).token.expires_in.toS;
    },
    getTokenType() {
     ...
     */
    async authorize( url , client_id, scope ) {
            ...
            const authorize_server = http.createServer(function (request, response) {
                ...
                switch (request.url) {
                    case "/styles.css":
                        ...
                        response.write(fs.readFileSync("./lib/styles.css"));
                        break;
                    case "/arrow_up.png":
                        response.writeHead(200, {"Content-Type": "image/png"});
                        response.write(fs.readFileSync("./lib/arrow_up.png"));
                        break;
                    default :
                        response.writeHead(200, {"Content-Type": "text/html"});
                        response.write(fs.readFileSync("./lib/index.html"));
     ...
     * @param client_secret
     */
    token( url, client_id, client_secret ) {
        return new Promise((resolve, reject) => {
     ...
     * @param client_secret
     */
    token_refresh( url, client_id, client_secret ) {
        return new Promise((resolve, reject) => {
